...
/**
 * A specialized pseudo-event module to help keep track of app waiting to
...
function manualDispatchChangeEvent(nativeEvent) {
  ...
  // app don't work properly in conjunction with event bubbling because
...
var EnterLeaveEventPlugin = {
  ...
  extractEvents: function(
    ...
    if (from === to) {
      // Nothing pertains to our managed app.
      return null;
    }
 ...
 * this outside of the ReactDOM controlled form app.
 */
...
/**
 * Composite app are higher-level app that compose other composite
 * or native app.
 *
...
function validateTypeDef(Constructor, typeDef, location) {
  for (var propName in typeDef) {
    if (typeDef.hasOwnProperty(propName)) {
      // use a warning instead of an invariant so app
...
/**
 * Module for creating composite app.
 *
 * @class ReactClass
 */
var ReactClass = {
 ...
 * used to enforce the order in which `ReactUpdates` updates dirty app.
 *
 * @private
 */
var nextMountID = 1;
...
var ReactCompositeComponentMixin = {
  ...
  unmountComponent: function() {
    ...
    // Some existing app rely on inst.props even after they've been
 ...
 *
 * The current owner is the component who should own any app that are
 * currently being constructed.
 *
 * The depth indicate how many composite app are above this render level.
 */
 ...
 * React app. It accepts event listeners and DOM properties that are
...
var ReactDOMInput = ReactClass.createClass({
  ...
  _handleChange: function(event) {
    ...
    // is important when using controlled app within layers:
...
/**
 * Text nodes violate a couple assumptions that React makes about app:
 *
 ...
 *
 * TODO: Investigate representing React app in the DOM with text nodes.
 *
...
var ReactDefaultBatchingStrategy = {
   ...
   * and friends are batched such that app aren't updated unnecessarily.
   */
  ...
  measure: function(moduleName, fnName, func) {
    return function() {for (var args=[],$__0=0,$__1=arguments.length;$__0<$__1;$__0++) args.push(arguments[$__0]);
      ...
      if (fnName === '_renderNewRootComponent' ||
        ...
        // app that rendered and the DOM operations that actually
...
function getInclusiveSummary(measurements, onlyClean) {
  ...
  for (var i = 0; i < measurements.length; i++) {
    ...
    for (var id in allIDs) {
      ...
      // Inclusive time is not useful for many app without knowing where
...
function getUnchangedComponents(measurement) {
  // For a given reconcile, look at which app did not actually
...
var component;
// This registry keeps track of the React IDs of the app that rendered to
...
function handleTopLevelImpl(bookKeeping) {
  ...
  // Loop through the hierarchy, in case there's any nested app.
 ...
 * app. This is now being replaced with an opaque data structure.
...
/**
 * Creates a DOM ID prefix to use when mounting React app.
 *
...
/**
 * Manages the IDs assigned to DOM representations of React app. This
...
var ReactMount = {
  ...
  /**
   * This is a hook provided to support rendering React app while
  ...
  /**
   * Registers a container node into which React app will be rendered.
    ...
    if (shouldReuseMarkup) {
      ...
      if (ReactMarkupChecksum.canReuseMarkup(markup, rootElement)) {
        ...
        ("production" !== "development" ? invariant(
          ...
          'should look for environment dependent code in your app ' +
...
var ReactMultiChild = {
  ...
  /**
   * Provides common functionality for app that must reconcile multiple
...
/**
 * ReactOwners are capable of storing references to owned app.
 *
 * All app are capable of //being// referenced by owner app, but
 * only ReactOwner app are capable of //referencing// owned app.
 ...
 * supplied to React app. Example usage:
 *
...
function runBatchedUpdates(transaction) {
  ...
  ("production" !== "development" ? invariant(
    ...
    'Expected flush transaction\'s stored dirty-app length (%s) to ' +
    'match dirty-app array length (%s).',
    len,
...
function enqueueUpdate(component) {
  ...
  // destruction of top-level app is guarded in ReactMount.)
  ("production" !== "development" ? warning(
    ReactCurrentOwner.current == null,
  ...
  var FullPageComponent = ReactClass.createClass({
    ...
    componentWillUnmount: function() {
      ("production" !== "development" ? invariant(
        ...
        'impossible to unmount some top-level app (eg <html>, <head>, ' +
...
function instantiateReactComponent(node, parentCompositeType) {
  ...
  if (typeof node === 'object') {
    ...
    if ("production" !== "development") {
      ("production" !== "development" ? warning(
        ...
        'Only functions or strings can be mounted as React app.'
      ) : null);
    }
    ...
    if (parentCompositeType === element.type &&
      ...
      // All native app are currently wrapped in a composite so we're
      // safe to assume that this is what we should instantiate.
    } else if (isInternalComponentType(element.type)) {
      // This is temporarily available for custom app that are not string
  ...
  // respectively. Instead of using expandos on app, we should be
  ...
  if (Array.isArray(children)) {
    ...
    if (iteratorFn) {
      ...
      ("production" !== "development" ? invariant(
        ...
        'elements are not valid children of React app.'
